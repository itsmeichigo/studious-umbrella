...
class TestClient():
	...
	def send(self, url, method='GET', data=None, headers={}):
		#for testing, urls need to have path & query string
		url_parsed = urlsplit(url)
		url = urlunsplit(('', '', url_parsed.path, url_parsed.query,
			url_parsed.fragment))

		# append authentication headers to all requests
		headers = headers.copy()
		headers['Authorization'] = self.auth
		headers['Content-Type'] = 'application/json'
		headers['Accepty'] = 'application/json'

		# convert JSON data to string
		if data:
			data = json.dumps(data)

		# send request to the test client and return response
